...
describe('glowsun test2', function () {
    beforeEach(() => {
        ...
        cy.visit('https://tps.thelattice.org/')
    ...
    it('Test on TPS Solar calculator', () => {
        cy.fixture('webtest2').then((data) => {
            for (let i = 0; i < rowsLength; i++) {
                ...
                cy.intercept('GET', '/api/saas/getAppliance').as(text_a)
                ...
                cy.intercept('GET', '/api/saas/microInverterMax').as(text_1)
                cy.contains(' CALCULATE MY SAVINGS ').click({ force: true })
                cy.wait('@' + text_1).its('response')
                    .should('deep.include', {
                        statusCode: 200
                    }).wait(3000)
                //---------------------------------- 1 st condition started----------------------------------------------------------------------------//
                cy.get('body').then(($ele) => {
                    if (data.rows[i].Electricity_Bill < 1000) {
                        cy.get('.mat-snack-bar-container').then(($val) => {
                            data_1.push($val.text(), '------------------------------------Condition completed------------------------------------------------------------')
                            cy.writeFile('cypress/fixtures/Scriptdata/retest.json', data_1)
                            cy.reload().wait(2000)
                            cy.window().then((win) => {
                    ...
                    //---------------------------------- 2 nd condition started----------------------------------------------------------------------------//
                        ...
                        cy.get('.mat-form-field').eq(2).click().type('1111111111')
